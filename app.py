...
elif not GOOGLE_API_KEY and LOCAL_DEV_API_KEY == "YOUR_GOOGLE_API_KEY_HERE":
    st.error("Google APIキーが設定されていません。StreamlitのSecrets、環境変数、またはコード内で有効なAPIキーを設定してください。", icon="🚨")
    st.stop()
elif not GOOGLE_API_KEY:
    st.error("Google APIキーの読み込みに失敗しました。設定を確認してください。", icon="🚨")
    st.stop()
try:
    genai.configure(api_key=GOOGLE_API_KEY)
except Exception as e:
    st.error(f"Google APIキーの設定またはGeminiライブラリの初期化に失敗しました: {e}", icon="🚨")
    st.stop()
